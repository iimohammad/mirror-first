...
**اگر پروکسی SNI روشن است، این به‌تنهایی کافی نیست.** ترافیک ۴۴۳ اول از
`sniproxy` رد می‌شود، و آنجا فیلتر لایه‌۴ است — TLS باز نمی‌شود، پس مسیر
(`/panel` در برابر `/repository`) اصلاً دیده نمی‌شود و نمی‌توان یکی را باز و
دیگری را بسته کرد. راه‌حل: پنل را از پورت مستقیم nginx صدا بزن که پروکسی را
دور می‌زند و nginx آنجا IP واقعی کلاینت را می‌بیند:

```bash
MIRROR_HTTPS_BIND=0.0.0.0:8443
MIRROR_PANEL_OPEN=1
```
...
| آدرس | دسترسی |
|---|---|
| `https://<domain>/…` (۴۴۳) | مخازن + passthrough — پشت `sni-allow.conf` |
| `https://<domain>:8443/panel/` | پنل — برای همه باز |
| `https://<domain>:8443/repository/…` | همچنان پشت `acl.conf` (اینجا IP واقعی دیده می‌شود) |

فایروال سرور باید ۸۴۴۳ را باز کند (`ufw allow 8443/tcp`).
